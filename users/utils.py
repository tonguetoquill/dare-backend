...
def get_callback_parameter(platform):
    """
    Convert AuthSourceChoice to callback parameter string.
    
    Args:
        platform: Platform name (AuthSourceChoice.DARE or AuthSourceChoice.SOCRATIC_BOOKS)
        
    Returns:
        str: Callback parameter from CallbackChoice enum
    """
    if platform == AuthSourceChoice.DARE:
        return CallbackChoice.DARE
    elif platform == AuthSourceChoice.SOCRATIC_BOOKS:
        return CallbackChoice.SOCRATIC_BOOKS
    
    return CallbackChoice.DARE  # Default to dare 