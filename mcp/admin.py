...
from mcp.models import GatewayFetch, MCPServer, MCPToolExecution, UserMCPConnection
...
class MCPToolExecutionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Context", {"fields": ("user", "server", "message", "conversation")}),
        ("Tool Call", {"fields": ("tool_name", "tool_arguments")}),
        (
            "Result",
...
@admin.register(GatewayFetch)
class GatewayFetchAdmin(admin.ModelAdmin):
    list_display = ("tool", "url", "doi", "user", "updated_at")
    list_filter = ("tool",)
    search_fields = ("url", "doi", "content")
    readonly_fields = (
        "user",
        "tool",
        "url",
    ...
    def has_add_permission(self, request):
        # Captured automatically by the gateway, not created by hand
        return False
