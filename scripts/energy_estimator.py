...
@dataclass
class EnergyReport:
    """Complete energy report combining stored impacts + derived stats."""
    # What we store
    impact: EnergyImpact

    # What we compute at read time
    stats: RelatableStats

    # Metadata
    provider: str
    model: str
    output_tokens: int
    source: str  # "ecologits" or "fallback"
...
def generate_energy_report(
    ...
    """
    Generate a complete energy report (impacts + relatable stats).

    Convenience function that combines compute_impact + compute_relatable_stats.

    Args:
        output_tokens: Output token count
        provider_name: DARE provider name
        model_name: Model identifier
        request_latency: Optional request latency in seconds

    Returns:
        Full EnergyReport
    """
    impact = compute_impact(output_tokens, provider_name, model_name, request_latency)
    stats = compute_relatable_stats(impact.energy_wh)

    source = "ecologits"
    ecologits_provider = PROVIDER_TO_ECOLOGITS.get(
        provider_name.lower(), provider_name.lower()
    )
    # Check if model was found in registry
    result = llm_impacts(
        provider=ecologits_provider,
        model_name=model_name,
...
def format_report(report: EnergyReport) -> str:
    """Format an EnergyReport as a human-readable string."""
    i = report.impact
    s = report.stats
    lines = [
        f"{'='*60}",
        f"  ENERGY REPORT: {report.model or report.provider}",
        f"  Source: {report.source} | Tokens: {report.output_tokens:,} out",
        f"{'='*60}",
        f"",
        f"  --- Stored Values (DB) ---",
        f"  Energy:        {i.energy_wh:.4f} Wh",
        f"  Carbon:        {i.carbon_g:.4f} g CO2e",
        f"  Water:         {i.water_ml:.4f} mL",
        f"",
        f"  --- Derived Stats (computed at read time) ---",
        f"  Phone battery:     {s.phone_battery_pct:.4f}%",
        f"  Google searches:   {s.google_searches_equiv:.2f}x",
        f"  LED bulb:          {s.led_bulb_seconds:.2f}s",
        f"  Netflix:           {s.netflix_seconds:.2f}s",
        f"  EV driving:        {s.ev_meters:.2f} meters",
        f"  Fridge:            {s.fridge_seconds:.2f}s",
        f"  Human thinking:    {s.human_thinking_seconds:.2f}s",
        f"{'='*60}",
    ]
    return "\n".join(lines)
